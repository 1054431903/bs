<template>
  <v-app id="app">
    <vc-navigation-authed-drawer v-model="drawer.show" :width="200">
    </vc-navigation-authed-drawer>
    <v-toolbar color="blue" dark app clipped-left fixed>
      <v-toolbar-title :style="$vuetify.breakpoint.smAndUp ? 'width: 500; min-width: 400px' : 'min-width: 72px'" class="ml-0 pl-3">
        <v-toolbar-side-icon @click.stop="drawer.show = !drawer.show"></v-toolbar-side-icon>
        <span class="hidden-xs-only">农事运营后台</span>
      </v-toolbar-title>
