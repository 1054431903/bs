 ...
 * @return {Function} 返回debounce函数
 */
function deferInvoke (fn) {
...
function diffTime (beginDateTime, endDateTime) {
  try {
    ...
    let timeDiff = new Date(
      new Date(endDateTime).getTime() - new Date(beginDateTime).getTime()
    )
    let dy = timeDiff.getFullYear() - 1970
    let dM = timeDiff.getMonth()
    let dd = timeDiff.getDate() - 1
    // let dy =
...
function deAreasCode (areacode) {
  if (
    !areacode ||
    areacode === '100000000000100000000000' ||
    areacode.length !== 24
  ) {
    return {}
  }
...
function getIconPath (rescourcename) {
  let extnames = global.constant.EXTNAMES
  if (!rescourcename) return '/static/task/wenjian.png'
  let extname = rescourcename.substr(
    rescourcename.lastIndexOf('.') + 1,
    rescourcename.length - 1
  )
  extname = extname.toUpperCase()
  let exts = ['images', 'videos', 'voices', 'words', 'excels', 'pdfs', 'others']
  let icon = ''
  exts.map(ext => {
    if (extnames[ext] && extnames[ext].suffix.includes(extname)) {
      icon = extnames[ext].icon
    }
  })
  !icon ? (icon = extnames.others.icon) : null
  return icon
}
