 ...
 * create by liangf 2018.2.10 v1.0.0
 * */
// export function excuteApis (requestParams, mod, submod, todo) {
//   requestParams = requestParams || {}
//   const user = global.helper.ls.get('user')
//   if (user) {
...
//   } else {
//     Object.assign(requestParams, { islogin: 0 })
//   }
//   const httpEntity = Object.assign(
//     { url: mod.baseUrl + submod + mod[submod][todo].url,
//       method: mod[submod][todo].method,
//       headers: mod[submod][todo].method === 'get' ? {'Content-Type': 'application/x-www-form-urlencoded'} : {'Content-Type': 'application/json'}
//     },
//     mod[submod][todo].method === 'get' ? {params: (function () {
//       let getParams = new URLSearchParams()
//       for (let p in requestParams) {
//         getParams.append(p, requestParams[p])
//       }
//       // if (user && user.loginorgid) {
//       //   getParams.append('orgid', user.loginorgid)
//       //   getParams.append('loginorgid', user.loginorgid)
//       //   getParams.append('operateid', user.id)
//       //   getParams.append('loginerid', user.supportid)
//       //   getParams.append('operateredittime', user.operateredittime)
//       // }
//       return getParams
//     })()} : {data: (function () {
//       let postParams = requestParams
//       if (user && user.loginorgid) {
//         Object.assign(postParams, {operateid: user.id, loginorgid: user.loginorgid, orgid: user.loginorgid, operateredittime: user.operateredittime, loginerid: user.supportid})
//       }
//       return postParams
//     })()})
...
export function excuteApis (requestParams, mod, submod, todo) {
  requestParams = requestParams || {}
  const httpEntity = Object.assign(
    { url: mod[submod][todo].paramstype ? (mod.baseUrl + submod + mod[submod][todo].url + '/' + requestParams) : mod.baseUrl + submod + mod[submod][todo].url,
      method: mod[submod][todo].method,
      headers: mod[submod][todo].method === 'get' ? {'Content-Type': 'application/x-www-form-urlencoded'} : {'Content-Type': 'application/json'}
    },
    // mod[submod][todo].method === 'get' ? {params: appendParams(requestParams, mod[submod][todo].method)} : {data: appendParams(requestParams, mod[submod][todo].method)}
    mod[submod][todo].paramstype ? null : (mod[submod][todo].method === 'get' ? {params: appendParams(requestParams, mod[submod][todo].method)} : {data: appendParams(requestParams, mod[submod][todo].method)})
  )
  return http(httpEntity)
}
