import {
  data as JsonSchemaData,
  util as JsonSchemaUtil
} from '@vehiclecloud/json-schema'
...
export function td (ctx, self, value, cv) {
  return {
    domProps: {
      innerHTML: String(cv !== undefined ? cv : '-') || '-'
    }
  }
}
...
export function accessStatusOf (v) {
  ...
  let items = JsonSchemaData.enumsOf(
    commonSchema.definitions.accessStatus.items
  )
...
export function accessDataText (d, code) {
  ...
  return (
    (JsonSchemaUtil.isDef(v) ? v : '-') +
    (sensorTypeVal(code, 'data.unit') || '')
  )
}

export function accessDataOf (device, sensorTypeCode) {
  return accessDataText(
    device.accessState &&
      device.accessState.data &&
      device.accessState.data[sensorTypeCode],
    sensorTypeCode
  )
}
...
export function accessDataWidget (sensorTypeCode) {
  return {
    tag: 'td',
    data: (ctx, el, item, v) => {
      return JsonSchemaVuetify.config.crud.table.helper.textData(
        ctx,
        el,
        item,
        v[sensorTypeCode] && v[sensorTypeCode].v
      )
    }
  }
}
...
export function paramsSchema (ctx, item) {
  let type = ctx.$store.state['base.device.type'].items.find(
    t => t._id.$oid === item.type.$oid
  )
  if (!type) return {}
  if (!type.paramTypes) return {}
  let paramTypes = type.paramTypes.map(pt =>
    ctx.$store.state['base.device.param_type'].items.find(
      t => t._id.$oid === pt.$oid
    )
  )
...
export const UiSchema = {
  TenantFarmland () {
    ...
    farmlandUI['ui:options'].map = (items, ctx, el, params) => {
      ...
      if (
        items &&
        params.farmland &&
        !items.some(item => item._id.$oid === params.farmland.$oid)
      ) {
        params.farmland = null
      }
      return oriMapFn(items)
    }
    farmlandUI['ui:options'].filter = (farmland, ctx, el, params) => {
      if (!farmland.tenant) return false
      if (params.tenant && params.tenant.$oid !== farmland.tenant.$oid) {
        return false
      }
      if (
        params.province &&
        params.province.$oid !== farmland.address.province.$oid
      ) {
        return false
      }
      if (params.city && params.city.$oid !== farmland.address.city.$oid) {
        return false
      }
      if (
        params.district &&
        params.district.$oid !== farmland.address.district.$oid
      ) {
        return false
      }
      if (
        params.town &&
        farmland.address.town &&
        params.town.$oid !== farmland.address.town.$oid
      ) {
        return false
      }
      return true
    ...
    tenantUI['ui:options'].map = (items, ctx, el, params) => {
      if (
        items &&
        params.tenant &&
        !items.some(item => item._id.$oid === params.tenant.$oid)
      ) {
        params.tenant = null
      }
    ...
    tenantUI['ui:options'].filter = (tenant, ctx, el, params) => {
      ...
      if (tenant.registeredAddress) {
        if (
          params.province &&
          (!tenant.registeredAddress.province ||
            (tenant.registeredAddress.province &&
              params.province.$oid !== tenant.registeredAddress.province.$oid))
        ) {
          return false
        }
        if (
          params.city &&
          (!tenant.registeredAddress.city ||
            (tenant.registeredAddress.city &&
              params.city.$oid !== tenant.registeredAddress.city.$oid))
        ) {
          return false
        }
        if (
          params.district &&
          (!tenant.registeredAddress.district ||
            (tenant.registeredAddress.district &&
              params.district.$oid !== tenant.registeredAddress.district.$oid))
        ) {
          return false
        }
      }
      ...
      if (
        farmlands &&
        !farmlands.some(f => f.tenant && f.tenant.$oid === tenant._id.$oid)
      ) {
        return false
      }
      return true
  ...
  Device (ctx, containOtherProvider = true) {
    ...
    result['ui:options'].filter = type => {
      ...
      if (ctx.params.deviceTypeTag && ctx.params.deviceTypeTag !== type.tag) {
        return false
      }
      return true
    }
...
export const Params = {
  ...
  DateRangeFactory () {
    ...
    return {
      startDate: {
        $numberLong: today.getTime() - Period.day * 6
      },
      stopDate: {
        $numberLong: today.getTime()
      }
    }
  }
}
...
export const Filter = {
  farmland (ctx) {
    return (
      ctx.params.farmland || {
        $in: ctx.params.farmlands.map(f => f._id)
      }
    )
  },
  deviceType (ctx) {
    return (
      ctx.params.deviceType || {
        $in: ctx.params.deviceTypes.map(t => t._id)
      }
    )
  }
}
