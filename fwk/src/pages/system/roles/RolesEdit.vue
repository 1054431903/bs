<template>
  <v-card class="page-content">
    <!-- <v-container style="border:1px solid #D6D6D6;background-color: #fff;"> -->
      <v-layout row style="border-bottom: 2px solid #DDDDDD; margin-top: 0;">
        <v-flex xs12 sm12 md12 lg12 xl12 style="padding-bottom: 10px">
          <v-flex >
            <span @click="back" style="cursor: pointer" >
              <v-icon dark left color="primary" >arrow_back</v-icon>编辑角色
            </span>
          </v-flex>
        </v-flex>
      </v-layout>
      ...
      <!--角色ID-->
      <!-- <v-layout row style="margin:50px 0;">
      ...
      <v-layout row justify-center>
        <v-flex xs8 sm8 md2 lg2 xl2>
          <v-subheader id="orgAlign">角色名称 *</v-subheader>
        </v-flex>
      ...
      <v-layout row justify-center>
        <v-flex xs8 sm8 md2 lg2 xl2 >
          <v-subheader text-xs-right id="orgAlign">权限配置</v-subheader>
        </v-flex>

        <v-flex xs8 sm8 md6 lg6 x6 style="padding-top: 20px;padding-bottom: 20px">
    ...
    methods: {
      ...
      fetchData () {
        ...
        excuteApis(requestParams, global.constant.userApisUrls, 'role', 'queryRole').then(data => {
          ...
          if (res.flag === 1 && res.data && res.data.length > 0) {
            ...
            if (res.data[0].permissionids) {
              this.selectedID = res.data[0].permissionids.split(',')
              for (let i = 0; i < _this.selectedID.length; i++) {
                let selectedPermission = _this.selectedID[i]
                setTimeout(function () { // 等dom创建完毕再将该角色拥有的权限状态设置为true(小心this的偏移)
                  _this.$refs[selectedPermission][0].checked = true
                }, 500)
              }
            }
          }
      ...
      save () {
        ...
        if (!!this.rolename && !!this.allID && this.allID.length > 0) {
          ...
          requestParams = this.allID ? Object.assign(requestParams, {permissions: this.allID.toString()}) : requestParams
          requestParams = this.orgid ? Object.assign(requestParams, {orgid: this.orgid}) : requestParams
