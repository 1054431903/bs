    ...
    v-form(v-model="valid" ref="form" lazy-validation autocomplete="off")
      ...
      v-layout(row, wrap)
        ...
        v-flex(xs12 sm12 md8 lg8 xl8)
          v-areaselect(:curAddressCode.sync="curAreaCode" :isReversal="true" :level=5 :isHideDetails="false")
...
import { isArray, centerOf, deAreasCode, enAreasCode } from '@/utils'
...
export default {
  data () {
    return {
      ...
      rules: {
        ...
        introduce: [
          (v) => this.introduceRule(v) || '不超过500个字符'
        ]
      },
      parcel: {
        id: '',
  ...
  watch: {
    'parcel.belongpeasantid': function (val, oldVal) {
      let peasent = this._.find(this.belongpeasants, ['id', val])
      if (peasent) {
        this.belongpeasantphone = peasent.linkphone
      } else {
        this.belongpeasantphone = ''
      }
    },
    selectDepartTree: function (val, oldval) {
      this.showSelectedTips = false
    },
    ...
    drawProfileDialog: function (v) {
      if (v) {
        this.isClickCloseBtn = true
        this.tmpLatlngs = this.polygon.latlngs
      } else {
        this.$set(this.polygon, 'editing', false)
        if (this.isClickCloseBtn) {
          this.polygon.latlngs = this.tmpLatlngs
        }
      }
    }
  },
  ...
  methods: {
    ...
    closeDepartSelect () {
      this.showSelectedTips = false
      this.selectDepartTree = false
    },
    closeCoordsOverrideTips () {
      this.isOverrideCoords = false
      this.drawProfileDialog = false
      this.polygon.latlngs = this.lastLatlngs
      if (this.btnFlag.indexOf('manualDraw') === 0) {
        this.polygon.editing = this.polygon.editing ? !this.polygon.editing : this.polygon.editing
      }
    },
    okCoordsOverride () {
      this.isOverrideCoords = false
    ...
    setValue (v) {
      ...
      if (v && v.length > 2) {
        ...
        this.map.layers.center = v[0]
        this.polygon.editing = true
        this.saveDrawProfile()
      }
    },
    // 根据地块ID查询地块信息
    fetchParcelData (parcelID) {
      ...
      // this.toast.show()
      excuteApis(Object.assign({}, { id: parcelID }), global.constant.parcelApis, 'parcel', 'query').then(data => {
        let res = data.data
        if (res.flag === 1) {
          this.parcel = res.data[0]
          if (this.parcel.responsibleuser && this.parcel.responsibleuser.length > 0) {
            this.selectedUser = this.parcel.responsibleuser
          } else {
            this.selectedUser = []
          }
          this.land = new CLand(this.parcel)
          this.belongpeasantphone = this.parcel.belongpeasant ? (this.parcel.belongpeasant.linkphone ? this.parcel.belongpeasant.linkphone : '') : ''
          // 根据地块位置区划代码查询5级行政区信息
          let provincecode = this.parcel['provincecode'] ? this.parcel['provincecode'] : null
          let citycode = this.parcel['citycode'] ? this.parcel['citycode'] : null
          let countycode = this.parcel['countycode'] ? this.parcel['countycode'] : null
          let towncode = this.parcel['towncode'] ? this.parcel['towncode'] : null
          let villagecode = this.parcel['villagecode'] ? this.parcel['villagecode'] : null
          // this.curAreaCode = this.land.getAddresscode()
          this.curAreaCode = enAreasCode(provincecode, citycode, countycode, towncode, villagecode) || '100000000000100000000000'
          // 根据地块ID查询地块轮廓经纬度
          // this.fetchParcelProfileLatLng(parcelID)
          // 查询地块轮廓
          this.land.landProfile().then(response => {
            let res = response.data
            let coords = res.data
            if (res.flag === 1) {
              if (isArray(coords) && coords.length > 0) {
                let lanlnts = []
                coords.forEach(item => {
                  lanlnts.push({ lat: item.latitude, lng: item.longitude })
                })
                this.$set(this.polygon, 'latlngs', lanlnts)
                this.$set(this.map.layers, 'center', centerOf(lanlnts))
                coords.length > 2 ? (this.importTitle = '(已完成)') : ''
                // 缓存地块轮廓信息
                this.lastLatlngs = lanlnts
              }
            }
          })
        }
    ...
    // 根据地块ID查询地块轮廓经纬度
    fetchParcelProfileLatLng (parcelID) {
      excuteApis(Object.assign({}, { parcelid: parcelID }), global.constant.parcelApis, 'parcelCoord', 'query').then(data => {
        let res = data.data
        if (res.flag === 1) {
          let coords = res.data
          if (coords.length > 0) {
            coords.forEach(this.dbLatLngToMapLatLng)
          }
        }
      })
    },
    // db经纬度转化为gis经纬度
    dbLatLngToMapLatLng (latLngObj) {
      this.polygon.latlngs[latLngObj.sort] = Object.assign({}, { lat: latLngObj.latitude, lng: latLngObj.longitude })
    },
    fetchLandOwership () {
      let requestParams = {
    ...
    saveParcel () {
      ...
      //   this.parcel.addresscode = this.curAreaCode
      //   Object.assign(this.parcel, {responsibleuserids: this.formatedSelectUserIds(this.selectedUser)})
      //   excuteApis(this.parcel, global.constant.parcelApis, 'parcel', 'update').then(response => {
      //     let res = response.data
      //     if (res.flag === 1) {
      //       this.$router.push({ path: 'successlandstips', query: { id: res.data.id, opflag: 1 } })
      //     } else {
      ...
      if (this.$refs.form.validate()) {
        // this.parcel.addresscode = this.curAreaCode
        console.log(this.curAreaCode)
        let { provincecode, citycode, countycode, towncode, villagecode } = deAreasCode(this.curAreaCode)
        // this.parcel.longitude = this.map.layers.center.lng
        // this.parcel.latitude = this.map.layers.center.lat
    ...
    showDrawProfileDialog () {
      // let code = this.village || this.town || this.city || this.county || this.province
      // if (!code) {
      //   this.prompt.title = '提示信息'
      //   this.prompt.body = '请选择地块位置'
      //   this.prompt.ok = false
      //   this.prompt.dialog = true
      //   return
      // }
      // if (!this.county) {
      //   this.prompt.title = '提示信息'
      //   this.prompt.body = '地块位置，至少需要选择三级行政区.'
      //   this.prompt.ok = false
      //   this.prompt.dialog = true
      //   return
      // }
      // let that = this
      // let county = this._.find(this.counties, {code: this.county})
      // let countyname = county ? county.name : '昆明市'
      // let requestParams = Object.assign({}, {name: countyname})
      // excuteApis(requestParams, global.constant.commonApiUrls, 'poslatlng', 'query').then(response => {
      //   let res = response.data
      //   if (res.flag === 1 && res.data.length > 0) {
      //     let data = res.data[0]
      //     let curCenterLatLng = Object.assign({}, {lat: data.lat, lng: data.lng})
      //     that.drawProfileDialog = true
      //     // that.polygon.latlngs[0] = curCenterLatLng
      //     that.map.layers.center = curCenterLatLng
      //   }
      // })
      // 获取中心坐标
      // 获取地块轮廓坐标
    ...
    recoverDrawProfile () {
      excuteApis(Object.assign({}, { parcelid: this.parcel.id }), global.constant.parcelApis, 'parcelCoord', 'delete').then(response => {
        let res = response.data
        if (res.flag === 1) {
          this.lastLatlngs = []
          ...
          this.$set(this.polygon, 'editing', false)
          // 设置地块中心点
          // this.$set(this.polygon, 'latlngs', [this.map.layers.center])
        }
      })
    },
    btnSaveDrawProfile () {
    ...
    saveDrawProfile () {
      ...
      this.drawProfileDialog = false
      let land = new CLand(this.parcel)
      // this.map.layers.center.lng = this.polygon.latlngs[0].lng
      // this.map.layers.center.lat = this.polygon.latlngs[0].lat
      this.map.layers.center = centerOf(this.polygon.latlngs)
      // this.parcel.longitude = this.polygon.latlngs[0].lng
      // this.parcel.latitude = this.polygon.latlngs[0].lat
      land.setCoords(null)
      land.setLongitude(this.parcel.longitude)
      land.setLatitude(this.parcel.latitude)
      // land.editLand().then(response => {
      //   let res = response.data
      //   if (res.flag === 1) {
      //     console.log('修改地块经纬度成功')
      //   } else {
      //     console.log('修改地块经纬度失败')
      //   }
      // })
      let coords = []
      for (let i = 0; i < this.lastLatlngs.length; i++) {
        let pos = Object.assign({}, { sort: i, longitude: this.lastLatlngs[i].lng, latitude: this.lastLatlngs[i].lat, parcelid: land.getId(), supportid: land.getSupportid() })
        coords.push(pos)
      }
      this.parcel.coords = coords
      land.setCoords(this.parcel.coords)
      if (land.getCoords().length < 3) return
      land.drawLand().then(response => {
        let res = response.data
        if (res.flag === 1) {
          console.log('批量修改地块坐标成功')
          if (this.polygon.latlngs.length > 2) {
            this.importTitle = '(已完成)'
          }
        } else {
          console.log('批量修改地块坐标失败')
        }
      })
    },
    Cancel () {
    ...
    nodeclick (data, node, tree) {
      if (data.sign && data.sign === '2') {
        ...
        this.showSelectedTips = true
      }
    },
    nodecheckclick (data, treestate) {
  ...
  created () {
    ...
    this.fetchParcelData(this.$route.query.id)
