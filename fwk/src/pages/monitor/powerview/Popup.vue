    ...
    <div>
      ...
      <div v-show="ctrlop"
        ...
        <div class="left">
          ...
          <v-layout row
                    class="right">
            <v-img class="button active"
                   @click="zoomSmall">
              <v-icon size="25">remove</v-icon>
            </v-img>
            <div style="width:30px;text-align:center;line-height:30px">缩放</div>
            <v-img class="button active"
                   @click="zoomBig">
              <v-icon size="25">add</v-icon>
            </v-img>
          </v-layout>
          <span>&nbsp;&nbsp;&nbsp;</span>
...
export default {
  ...
  methods: {
    ...
    ctrRequest (direction) {
      axios.post(`${url}/api/lapp/device/ptz/start`, JSON.stringify({
        ...
        setTimeout(() => {
          axios.post(`${url}/api/lapp/device/ptz/stop`, JSON.stringify({
            deviceSerial: this.device.code,
            channelNo: 1,
            direction: direction
          })
    ...
    capture () {
      ...
      axios.post(`${url}/api/lapp/device/capture`, obj).then(response => {
        if (response.data['code'] === 200 && response.data.data['osspicUrl'].length > 0) {
          let list = []
          let time = new Date().getTime()
          for (let i = 0; i < response.data.data['osspicUrl'].length; i++) {
            let p = mongo.db('iot').collection('tenant.device_powerview_data').insert({
