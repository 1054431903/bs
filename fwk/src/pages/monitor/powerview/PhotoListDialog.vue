...
export default {
  ...
  methods: {
    ...
    getMore (skip) {
      mongo.db('iot').collection('tenant.device_powerview_data').find({ filter: { device: { '$oid': this.device._id.$oid } }, sort: { 'createdOn': -1 }, 'limit': this.pageSize, 'skip': skip * this.pageSize }).then(data => {
        console.log(data)
