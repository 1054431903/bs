                ...
                <v-flex xs6 class="mb-3 pr-3" v-for="col in collectors" :key="col.code">
                  <v-layout>
                    ...
                    <strong style="flex-shrink: 0;">
                      <span :class="{'blue--text': device.accessState.data[col.code].as === 1, 'warning--text': device.accessState.data[col.code].status !== 1}" :title="accessStatus[device.accessState.data[col.code].as] || '未知状态'">{{ accessStatus[device.accessState.data[col.code].as] || '未知状态' }}</span>
                    </strong>
                  </v-layout>
                </v-flex>
              </v-layout>
...
import { sensorTypeVal, accessDataText, accessDataOf } from '@/pages/monitor/util/util'
