  ...
  <div class="devices-search-panel">
      ...
      <v-card-title class="py-0">
        <form class="layout row"
          ...
          <v-btn type="submit"
                 small
                 color="info"
                 class="ml-3 btn--search">搜索</v-btn>
        </form>
      </v-card-title>
      <v-card-actions class="card__actions-tags">
        <v-btn-toggle v-model="params.tag"
                      :class="{'btn-toggle--right': tags.right}">
          <template v-for="(tag, i) in tags.items">
            <v-btn flat
                   :value="tag.value"
                   :key="tag.value"
                   @click="params.tag = tag.value">{{ tag.text }}</v-btn>
            <div class="divider-v"
                 :key="'d_' + tag.value"
                 v-if="(i + 1) % 3 && i !== tags.items.length - 1"></div>
            <template v-if="!((i + 1) % 3)">
              <a class="a-slider"
                 :key="'ar-' + i"
                 v-show="!tags.right"
                 @click="tags.right = true">
                <v-icon>keyboard_arrow_right</v-icon>
              </a>
              <a class="a-slider a-slider-left"
                 :key="'al-' + i"
                 v-show="tags.right"
                 @click="tags.right = false">
    ...
    <v-card class="mt-3 pt-2">
      <v-list two-line
              class="pb-0">
        <v-list-tile>
          <vc-schema-render :tag="null"
                            :scope="1"
                            :elements="list.farmland"
                            :item="farmland"></vc-schema-render>
        </v-list-tile>
        <v-divider></v-divider>
      </v-list>
      <div class="scrollbar-thin container-devices">
        <v-list two-line>
          <template v-for="device in value">
            <v-list-tile :key="device._id.$oid"
                         :value="device === currentDevice"
                         @click="select(device)">
              <vc-schema-render :tag="null"
                                :scope="1"
                                :elements="list.device"
                                :item="device"></vc-schema-render>
            </v-list-tile>
        ...
        <div class="text-xs-center pt-3 pb-4">
          <v-pagination :length="page.length"
                        total-visible="6"
                        v-model="page.current"
                        circle
                        prev-icon="arrow_back"
                        next-icon="arrow_forward"
                        color="light-blue lighten-1"></v-pagination>
        </div>
      </div>
    </v-card>
  </div>
...
export default {
  ...
  methods: {
    ...
    query () {
      ...
      mongo.db('iot').collection('tenant.device').find({
        filter,
...
.devices-search-panel {
  ...
  .container-devices {
    ...
    .v-pagination {
      ...
      .v-pagination__item.pagination__item--active {
        color: white;
      }
      .v-pagination__navigation {
        .icon {
