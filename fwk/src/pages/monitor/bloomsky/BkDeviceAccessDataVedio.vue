<template>
  <v-layout row
    ...
    <div style="padding-left:40px;flex:1;max-width:800px;padding-top:60px;">
      <v-date-picker v-model="veido_date"
                     :max="maxDate"
                     :min="minDate"
                     full-width
                     no-title
...
export default {
  ...
  watch: {
    'veido_date' (v) {
      mongo.db('iot').collection('tenant.device_bloomsky_video_data').find({ filter: { 'device': { '$oid': this.device._id.$oid }, time: new Date(new Date(v).toLocaleDateString()).getTime() } }).then(d => {
        this.aplayer.source = ''
        if (d.items.length > 0) {
  ...
  created () {
    ...
    this.minDate = this.fmt_Date(nowTime - 6 * Period.day)
    mongo.db('iot').collection('tenant.device_bloomsky_video_data').find({ filter: { 'device': { '$oid': this.device._id.$oid }, time: nowTime } }).then(d => {
      this.aplayer.source = ''
      if (d.items.length > 0) {
        this.aplayer.source = d.items[0].data.v.startsWith('http:') ? d.items[0].data.v : 'http://' + d.items[0].data.v
      }
