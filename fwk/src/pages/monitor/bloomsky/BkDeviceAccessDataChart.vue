<template style="background-color:#fff">
  <v-layout row wrap v-if="tableFlag && tf">
   <v-flex xs12  v-if="sensorTypeArr['61'] && f8">
    <v-card dark color="white" style="max-height:200px;">
        <ve-line :data="chartData" height="200px" :settings="chartSettings" :colors="['#A9E5E1']" :grid="grid4" :legend-visible="false"></ve-line>
    </v-card>
    </v-flex>
    <v-flex xs12  v-if="sensorTypeArr['62'] && f1">
    <v-card dark color="white" style="max-height:200px;">
    <ve-line :data="chartData1" height="200px" :settings="chartSettings1" :colors="['#D6D7D9']" :grid="grid5" :legend-visible="false"></ve-line>
    </v-card>
    </v-flex>
    <v-flex xs12 v-if="sensorTypeArr['63'] && f2">
    <v-card dark color="white" style="max-height:200px;">
        <ve-line :data="chartData2" height="200px" :settings="chartSettings2" :colors="['#A9E5E1']" :grid="grid1" :legend-visible="false"></ve-line>
    </v-card>
    </v-flex>
    <v-flex xs12  v-if="sensorTypeArr['64'] && f7">
    <v-card dark color="white" style="max-height:200px;">
        <ve-line :data="chartData7" height="200px" :settings="chartSettings7" :yAxis= "w_direction" :tooltip="tooltip" :colors="['#F5784E']" :grid="grid3" :legend-visible="false"></ve-line>
    </v-card>
    </v-flex>
    <v-flex xs12  v-if="sensorTypeArr['67'] && f4">
    <v-card dark color="white" style="max-height:200px;">
        <ve-line :data="chartData4" height="200px" :settings="chartSettings4" :colors="['#95C4E3']" :grid="grid2"  :legend-visible="false"></ve-line>
    </v-card>
    </v-flex>
    <v-flex xs12 v-if="sensorTypeArr['69'] && f5" >
    <v-card dark color="white" style="max-height:200px;">
        <ve-line :data="chartData5" height="200px" :settings="chartSettings5" :colors="['#C0C2C4']" :grid="grid4" :legend-visible="false"></ve-line>
    </v-card>
    </v-flex>
    <v-flex xs12  v-if="sensorTypeArr['68'] && f6">
    <v-card dark color="white" style="max-height:200px;">
        <ve-line :data="chartData6" height="200px" :settings="chartSettings6" :colors="['#F9BBA9']" :grid="grid1" :legend-visible="false"></ve-line>
    </v-card>
    </v-flex>
    <v-flex xs12 v-if="sensorTypeArr['66'] && f3" >
    <v-card dark color="white" style="max-height:200px;">
        <ve-line :data="chartData3" height="200px" :settings="chartSettings3" :colors="['#D599B0']" :grid="grid1" :legend-visible="false"></ve-line>
    </v-card>
    </v-flex>
  </v-layout>
  ...
  <div v-else style="text-align:center;margin-top:25%;min-height:200px;">
    暂无历史数据
  </div>
</template>
<script>
...
export default {
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: function (params, ticket, callback) {
          let tmp = params[0].name + '</br>风向:&nbsp&nbsp' + windfomat(params[0].value)
          return tmp
        }
      },
      tableFlag: false,
  ...
  methods: {
    ...
    list: function (start, end) {
      ...
      let uvrows = []
      let wdirows = []
      let ps = mongo.db('iot').collection('tenant.report.device_access_data_summary').find({filter: {device: {'$oid': this.device._id.$oid}, period: 6, time: { '$gte': start, '$lte': end }}, sort: {'time': 1}, limit: 12000})
      ps.then(items => {
        _this.f1 = false
        _this.f2 = false
        _this.f3 = false
        ...
        if (wdirows.length > 0) {
          if (this.grid3) this.grid3.left = 13
          _this.f7 = true
        } else {
          if (this.grid3) this.grid3.left = 50
          _this.f7 = false
        }
        if (pressrows.length > 0) {
          if (this.grid2) this.grid2.left = 24
          _this.f4 = true
        } else {
          if (this.grid2) this.grid2.left = 50
          _this.f4 = false
        }
        if (sdrows.length > 0) {
          if (this.grid5) this.grid5.left = 34
          _this.f1 = true
        } else {
          if (this.grid5) this.grid5.left = 50
          _this.f1 = false
        }
