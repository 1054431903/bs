<template>
  <div>
    <v-data-table v-bind="table" :pagination.sync="table.pagination" no-data-text="未找到相应数据." no-results-text="没有匹配的数据.">
      <template slot="items" slot-scope="{ item, index }">
        <td v-for="h in table.headers" :key="h.text" :class="{'yellow lighten-3': isException(item, h.sensorType) }">{{ h.td(item) }}</td>
      </template>
...
import DeviceAccessData from '@/components/device/mixins/device-access-data'
  ...
  methods: {
    refresh () {
      ...
      (this.sensorTypesInTable || this.sensorTypes).forEach(st => {
        headers.push({ ...commonTableHeader, text: st.name + (st.data.unit ? `(${st.data.unit})` : ''), sensorType: st, td: data => (data.dataInfo._data[st.code] && data.dataInfo._data[st.code].v) || '-' })
      })
      if (this.showAccessCodes) {
        headers.push({ ...commonTableHeader, text: '状态', td: data => (this.errorData && this.errorData[data.dataTime] && this.errorData[data.dataTime].code && this.errorData[data.dataTime].code.filter(c => this.skipAccessStatus ? accessStatusCodeMap[this.skipAccessStatus].includes(c) : true).map(c => AccessDataCodes[c]).join(', ')) || '正常' })
      }
        ...
        this.fetchErrorData().then(() => {
          ...
          if (!this.skipAccessStatus) {
            ...
            this.table.items = dataList.filter(d =>
              !(this.errorData && this.errorData[d.dataTime] && this.errorData[d.dataTime].status.includes(this.skipAccessStatus))
            )
          }
          this.table.loading = false
        })
    ...
    isException (item, sensorType) {
      return sensorType &&
        ...
        this.errorData[item.dataTime].data[sensorType.code].status > 3
    },
    fmtDate (time) {
      let d = new Date(+time)
