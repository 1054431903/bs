  ...
  <v-container class="access-data pt-0">
    <v-layout row>
      <vc-schema-form tag="div" v-bind="search" v-model="params">
        <v-flex xs5 style="align-self: flex-end">
          <div style="margin-bottom: 2px;height:30px;margin-left:10px;background-color:#fff;">
            <button  @click="quickSetDate(0)" class="tbutton" :class="tclz">今天</button>
            <button  @click="quickSetDate(7)" class="tbutton" :class="wclz">7天</button>
            <button  @click="quickSetDate(30)" class="tbutton" :class="mclz">30天</button>
          </div>
        </v-flex>
      </vc-schema-form>
      <v-layout align-end justify-end>
        ...
        <v-flex v-if="typeCode !== 105" class="text-xs-center flex--tab" :class="{ active: tab.current === 2 }">
          <a @click="tab.current = 2" class="black--text">视频</a>
        </v-flex>
      </v-layout>
    </v-layout> 
    
    <div class="pt-4 px-0" v-if="tab.current === 0">
      <bk-device-access-data-chart v-if="typeCode !== 105" :device="device" :start="startTime" :end="stopTime"></bk-device-access-data-chart>
      <rain-device-access-data-chart v-else :device="device" :start="startTime" :end="stopTime"></rain-device-access-data-chart>
    </div>
    <div class="card--table pt-3 px-0" v-if="tab.current === 1">
      <bk-device-access-data-table v-if="typeCode !== 105" :device="device" :start="startTime" :end="stopTime"></bk-device-access-data-table>
      <rain-device-access-data-table v-else :device="device" :start="startTime" :end="stopTime"></rain-device-access-data-table>
    </div>
    <div class="card--table pt-3 px-0" v-if="tab.current === 2">
...
export default {
    ...
    return {
      ...
      search: {
        ...
        uiSchema: {
          ...
          // startTime: { 'ui:props': { singleLine: true, hideDetails: true, min: minDate, max: JsonschemaData.longToDateStr(this.params.stopTime) }, 'ui:class': { 'mr-3': true } },
          startTime: { 'ui:props': (ctx, el, item) => ({ singleLine: true, hideDetails: true, min: minDate, max: JsonschemaData.longToDateStr(item.stopTime) }), 'ui:class': { 'mr-3': true } },
          stopTime: {
      ...
      params: {
        startTime: { $numberLong: new Date(new Date().toLocaleDateString()).getTime() },
  ...
  methods: {
    quickSetDate (internal) {
      ...
      this.smaxDate = JsonschemaData.longToDateStr(stopTime)
    }
  },
  components: {
