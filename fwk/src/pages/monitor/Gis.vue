...
import { accessDataOf, Schema, Dependencies, UiSchema, Params } from './util/util'
    ...
    ['city', 'district', 'town'].forEach(name => {
      ...
      let filter = (option, ctx, el, item) => {
        return oriFilter(option, ctx, el, item) &&
          this.$store.state['tenant.farmland'].items.some(f => f.address && f.address[name] && f.address[name].$oid === option._id.$oid)
      }
...
const Farmlands = {
  ...
  methods: {
    showFarmlands () {
      ...
      this.farmlandsHoverReset()
      let items = this.farmlands.items = (this.search.params.farmland ? [this.search.params.farmlands.find(f => f._id.$oid === this.search.params.farmland.$oid)] : this.search.params.farmlands).filter(f => !!f.gis)
      if (items.length) {
    ...
    farmlandsQueryCounts () {
      let tf = this.$store.state['tenant.farmland'].items.filter(
        e => e.tenant && e.tenant.$oid === global.helper.ls.get('user').loginorgid).map(m => ({ $oid: m._id.$oid }))
      return this.$store.state['tenant.farmland'].promise.then(() => {
        this.farmlands.counts.promise = mongo.db('iot').collection('tenant.device').aggregate([
          { $match: { status: 1, _removed: { $exists: false }, farmland: { $in: tf } } },
