<template lang="pug">
v-flex(xs12)
  v-select(v-if="['select', 'select2'].includes(field.type)", :items='field.choices', v-model='model', v-bind='field', :label="$t(field.label)")
  ...
  v-text-field(v-else-if="['textserach'].includes(field.type)", v-model='model', v-bind='field', :label="$t(field.label)" :placeholder="$t(field.placeholder)" type="text" append-icon="search")
