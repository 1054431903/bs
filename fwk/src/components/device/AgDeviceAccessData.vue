<template>
  <v-container class="access-data pt-0">
      ...
      <vc-schema-form tag="div"
        ...
        <v-flex xs5
          ...
          <v-btn-toggle style="margin-bottom: -4px;">
            <v-btn flat
                   @click="quickSetDate(0)">今天</v-btn>
            <v-btn flat
                   @click="quickSetDate(7)">7天</v-btn>
            <v-btn flat
                   @click="quickSetDate(30)">30天</v-btn>
          </v-btn-toggle>
        </v-flex>
      </vc-schema-form>
      <v-layout align-end
    ...
    <div class="pt-4 px-0"
         v-if="tab.current === 0">
      <ag-device-access-data-chart :device="device"
                                   :start-time="startTime"
                                   :stop-time="stopTime"></ag-device-access-data-chart>
    </div>
    <div class="card--table pt-3 px-0"
         v-if="tab.current === 1">
      <ag-device-access-data-table :device="device"
                                   :start-time="startTime"
                                   :stop-time="stopTime"
                                   :skip-access-status="skipAccessStatus"
                                   :marker-access-status="markerAccessStatus"
                                   :show-access-codes="showAccessCodes"
                                   :sensor-types-in-table="sensorTypesInTable"></ag-device-access-data-table>
    </div>
  </v-container>
</template>
