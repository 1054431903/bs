<template>
  <div>
    <div v-for="(sensorType, idx) in sensorTypes"
         :key="sensorType._id.$oid">
      <h4 class="ml-4"
          :class="{'mt-4': idx > 0}">{{ sensorType.name }}</h4>
...
export default {
  ...
  methods: {
    refresh () {
      ...
      mongo.db('iot').collection('tenant.report.device_access_data_summary').find({
        filter: {
